...
일반적으로 최적화 문제는 어떤 함수 $f:U \subset \R^n \to \R$ 에 대해 아래의 $\bf{\theta}^\ast$ 문제를 구하는 것으로 주어진다.
...
- 제약조건을 만족하는 $\mathcal{C}\subset \R^n$ 를 **가능해 영역(feasible region)** 이라고 한다. @eq-ML_optimization_optimization_problem 과 같은 최적화 문제에 대해 $\mathcal{C}$ 는 아래와 같다.
$$
...
::: {#thm-optimization_convex_solution_set}
...
$\bf{\theta} = \begin{bmatrix}\theta_1 & \cdots & \theta_n \end{bmatrix}^T \in \R^n$, $\bf{a} \in \R^n$, $\bf{A}=\mathcal{M}_{m \times n}(\R)$ 일 때 $\bf{\theta}$ 에 대한 스칼라 함수의 gradient 는 다음과 같다.
...
으로 주어졌다면 @exm-optimizatin_object_function_Square 로 부터
...
::: {#thm-optimization_lagrange_multipiler}
