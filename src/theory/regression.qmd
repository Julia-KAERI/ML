...
### 선형회귀와 설계행렬 {#sec-ML_regression_design_matrix}
...
$$ {#eq-ML_linear_regression_master_equation}


노름 가운데 유클리드 노름, 즉 $L_2$ 노름 혹은 $L_1$ 노름을 사용한다. $L_p$ 노름을 사용했을 경우 @eq-ML_linear_regression_master_equation 는 다음과 같이 표기된다.

$$
\bf{\theta}^\ast = \argmin_\bf{\theta} \left\|\bf{y}- \bf{\Phi \theta}\right\|_p
$$ {#eq-ML_linear_regression_master_equation_with_p}
...
$$ {#eq-ML_linear_regression_master_equation_with_L2}

이며 $\argmin_\bf{\theta}\|\bf{y}-\bf{\Phi\theta}\|_2^2$ 가 $\sqrt{\cdots}$ 가 없어 미분 계산이 훨씬 간단해 지기 때문에 노름의 제곱을 많이 사용한다.
 
</br>

### 학습/훈련과 예측{#sec-ML_regression_training_and_prediction}

우리가 이미 획득한 입력 벡터의 집합 $\{\bf{x}^{(i)}\}$ 와 레이블(label) $\{y^{(i)}\}$ 로 이루어진 데이터셋 $\{(\bf{x}^{(i)},\, y^{(i)})\}$, 그리고 데이터를 잘 설명할것이라고 기대되는 함수 $f(\bf{x},\,\bf{\theta})$ 에 대해 @eq-ML_linear_regression_master_equation 의 $\bf{\theta}^\ast$ 를 얻는 것을 훈련 혹은 학습 이라고 한다. 이 $\bf{\theta}^\ast$ 에 대해 $\bf{x}$ 의 상황에서 $f(\bf{x};\bf{\theta}^\ast)$ 의 값을 얻을것이라고 기대한다. 즉 우리는 주어진 데이터를 학습시켜 미지의 상황을 예측 할 수 있다. 이것은 비단 선형 회귀 뿐만 아니라 모든 통계학적 처리 및 기계학습의 과정에 공통된다.
...
### 최소제곱합 {#sec-ML_regression_least_square_sum}

@eq-ML_linear_regression_master_equation_with_L2 은 보통 데이터와 모델값의 오차에 대한 제곱합 오차 함수 $E(\bf{\theta})$ 이다. 
...
$$ {#eq-ML_linear_regression_solution_L2_norm}
...
### 1차원 선형 회귀 {#sec-ML_regression_1d_linear}
...
이다. $L_2$ 노름의 경우 $\bf{\theta}^\ast$ 는 @eq-ML_linear_regression_solution_L2_norm 로 부터 다음과 같다는 것을 안다.
...
### 다항 회귀 {#sec-ML_regression_polynomial_regression}
...
### 비선형 회귀 {#sec-ML_regression_nonlinear_regression}
...
### Overfitting {#sec-ML_regression_overfitting}



주어진 데이터 에 대한 2차, 4차, 5차, 9차 다항식에 대한 다항회귀의 결과는 아래와 같다.

![다항회귀](notebooks/overfit.png){#fig-ML_regression_overfit width=400}
...
### Reguralization {#sec-ML_regression_reguralization}
...
$$ {#eq-ML_regression_regulaization}

로 정하는 것이다. @eq-ML_regression_regulaization 의 $\lambda \|\bf{\theta}\|$ 는 $\|\bf{\theta}\|$ 값이 큰 경우에서 $\bf{\theta}^\ast$ 가 결정되는 것을 방해한다. 뒤의 $\lambda \|\bf{\theta}\|$ 의 노름은 $L_1$ 혹은 $L_2$ 가운데 선택하며 앞의 $\|\bf{y}-\bf{\Phi \theta}\|$ 의 노름과 같을 필요는 없다. $L_2$ 노름의 경우를 **Ridge regression** 이라고 하고 $L_1$ 노름의 경우는 **LASSO** (least absolute shrinkage and selection operator) 라고 한다.
