...
$$
\text{bias}(\hat{\bf{\theta}}_m) = \mathbb{E}(\hat{\bf{\theta}}_m) -\bf{\theta}.
...
$$
\hat{\mu}_m = \dfrac{1}{m} \sum_{i=1}^m x^{(i)}.
$$
...
가우시안 분포의 분산 $\sigma^2$ 에 대한 일반적인 추정값은 표본 분산 $\hat{\sigma}^2_m$ 으로 다음과 같다,.

$$
\hat{\sigma}_m = \dfrac{1}{m} \sum_{i=1}^m  \left(x^{(i)} - \hat{\mu}_m\right)^2
$$
...
&= \mathbb{E} \left[\dfrac{1}{m}\sum_{i=1}^m \left(x^{(i)} - \mu\right)^2\right]  +\mathbb{E} \left[\dfrac{1}{m}\sum_{i=1}^m(\hat{\mu}_m- \mu)^2\right]- 2\mathbb{E}\left[\dfrac{1}{m}\sum_{i=1}^m\left(x^{(i)} - \mu\right) \left(\hat{\mu}_m - \mu\right)\right] \\[0.3em]
\end{aligned}
$$
...
이며, 따라서 @eq-ML_ML_unbiased_sample_variance 애서 정의된 $\tilde{\sigma}_m^2$ 를 **불편 표본분산(unbiased sample variance)** 라고 한다.
